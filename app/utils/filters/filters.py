...
def drawing(img):
    pencil_gray, pencil_color = cv2.pencilSketch(
        img, sigma_s=100, sigma_r=0.07, shade_factor=0.05
    )  # inbuilt function to generate pencil sketch in both color and grayscale
    return pencil_gray
        ...
        for j in range(width):
            if np.random.rand() <= thresh:
                if np.random.randint(2) == 0:
                    ...
                    gray[i, j] = max(
                        gray[i, j] - np.random.randint(0, 64), 0
...
def sidebyside_four(imageleftup, imagerightup, imageleftdown, imagerightdown):
    newimheight = (
        max(
            imageleftup.shape[0],
            imagerightup.shape[0],
            imageleftdown.shape[0],
            imagerightdown.shape[0],
        )
        * 2
    )
    newimwidth = (
        max(
            imageleftup.shape[1],
            imagerightup.shape[1],
            imageleftdown.shape[1],
            imagerightdown.shape[1],
        )
        * 2
    )
...
def timewarp(img, previousFr):
    try:
        b_0, g, r = cv2.split(previousFr[4])
        b, g_20, r = cv2.split(previousFr[12])
        b, g, r_40 = cv2.split(previousFr[20])
        img = cv2.merge([b_0, g_20, r_40])
        # blended = cv2.addWeighted(frame,0.25,merged,0.75,0)
    except:
        pass
    return img
