...
def mediapipe_result(holistic, image):
...
def red_nose(holistic, img):
    try:
        results = mediapipe_result(holistic, img)
        nose = results.face_landmarks.landmark[1]
        img = cv2.circle(
            img,
            (int(nose.x * img.shape[1]), int(nose.y * img.shape[0])),
            int((-400 * nose.z + 1)),
            (0, 0, 255),
            -1,
        )
...
def carnival(holistic, img):
    ...
    img2 = img2_import[:, :, :3]
    alpha = np.array(
        [img2_import[:, :, 3], img2_import[:, :, 3], img2_import[:, :, 3]],
        dtype="uint8",
    )
    ...
    try:
        ...
        mask_fake = cv2.warpAffine(
            mask_fake, M, (mask_fake.shape[1], mask_fake.shape[0])
        )

        img_fake = img_fake.astype("float")
        mask_fake = mask_fake.astype("float") / 255

        img = (mask_fake * img_fake + (1.0 - mask_fake) * img1).astype("uint8")
